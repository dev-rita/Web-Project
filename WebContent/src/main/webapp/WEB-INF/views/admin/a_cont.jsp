...
function getAllList(){
	...
	$.getJSON("/controller/all/"+b_no,function(data){
		...
		$(data).each(function(){//each()함수로 반복
			...
			+"<div class='content-body panel-body pull-left'>"
			+"<a id='choose' onclick='choose()' href='javascript://' class='note-vote-btn note-select-btn note-selected' data-id='2516028' data-type='select'>"
	        +"<i class='fa fa-check'></i></a>"	      
	        +"<c:if test="+this.r_choose==0+"><a id='choose' onclick='choose()' href='javascript://' class='note-vote-btn note-select-btn note-selected' data-id='2516028' data-type='select'><i class='fa fa-check'></i></a></c:if>"         
		    +"<div class='avatar clearfix avatar-medium '>"		    
...
node.setAttribute("class","fa fa-times");
function choose(){
	if(!document.getElementById('choose').classList.contains('note-selected')){	
		document.getElementById('choose').classList.toggle('note-selected');
		document.getElementById('choose').appendChild(node)
	}else{
		document.getElementById('choose').classList.toggle('note-selected');
		document.getElementById('choose').removeChild(node)
	}
}
</script>
